...
export const foodTrackerAPI = {
  ...
  // Charger une semaine - retourne null si pas de données (au lieu d'erreur)
  async loadWeek(weekStart) {
    ...
    try {
      ...
      // Si des données existent, les retourner
      ...
      // Aucune donnée trouvée - c'est normal pour les nouvelles semaines
      return null
    } catch (error) {
  ...
  // Vérifier si une semaine existe
  async weekExists(weekStart) {
    if (!supabaseClient) {
      console.warn('Supabase non disponible')
      return false
    }
